...
\end{itemize}
All these elements should be serialized into a single string which could be used as the activation key by iSmartNotes. Therefore, no matter if the user decides to use iSmartNotes for the first time or has just changed the machine, they will only be required to login to SmartNotes using Google Account, download the application and pass the activation code. *****To make a better comparison between this idea and how it work using a classical account system is be...komary tna ide    
