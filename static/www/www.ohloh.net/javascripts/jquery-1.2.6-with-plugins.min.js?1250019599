...
jQuery.fn.extend({typeWatch:function(A){waitTextbox(this,A)}});function waitTextbox(B,A){B.each(function(){thisEl=jQuery(this);var E="typewatch_"+typewatch_uid++;var G={timer:null,text:null,cb:null,el:null,wait:null};this.typewatchid=E;if(this.type.toUpperCase()=="TEXT"||this.nodeName.toUpperCase()=="TEXTAREA"){window[E]=G;var I=window[E];var H=750;var C=function(){};var D=true;var F=true;if(A){if(A.wait!=null){H=parseInt(A.wait)}if(A.callback!=null){C=A.callback}if(A.highlight!=null){D=A.highlight}if(A.enterkey!=null){F=A.enterkey}}I.text=thisEl.val().toUpperCase();I.cb=C;I.wait=H;I.el=this;if(D){thisEl.focus(function(){this.select()})}thisEl.keydown(function(J){var L=this.typewatchid;var K=window[L];if(J.keyCode==13&&this.type.toUpperCase()=="TEXT"){clearTimeout(K.timer);K.timer=setTimeout('typewatchCheck("'+L+'", true)',1);return !F}clearTimeout(K.timer);K.timer=setTimeout('typewatchCheck("'+L+'", false)',K.wait)})}})}function typewatchCheck(D,B){var C=window[D];var A=$(C.el).val();if((A.toUpperCase()!=C.text)||(B)){C.text=A.toUpperCase();C.cb(A)}}var typewatch_uid=0;
